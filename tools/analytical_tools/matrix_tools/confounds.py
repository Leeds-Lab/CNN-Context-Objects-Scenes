...
def confound_expansion(confound_matrix, dimension_number, expansion_number):
    expansion_matrix = np.empty(((dimension_number*expansion_number), (dimension_number*expansion_number)))
    confound_matrix = confound_matrix[:dimension_number].T
    ...
    for i in range((dimension_number*expansion_number)):
        for j in range((dimension_number*expansion_number)):
...
def create_confound_matrix():
    ...
    context_confounds = np.load(CONTEXT_CONFOUNDS)
    ...
    return context_confounds

# Get Context Confound submatrix 
def context_confound_submat(context_confounds, k, submatrix_data):

    # concatenate outvalues in confound matrix in the same way as was done in original correlation matrix
    ...
    for j in range(CONTEXTS):
        ...
        try:
            temp = np.vstack([temp,context_confounds[CONTEXT_EXEMPLARS*j : CONTEXT_EXEMPLARS*j + CATEGORY_EXEMPLARS ,CATEGORY_EXEMPLARS*(2*k + 1) : CATEGORY_EXEMPLARS*(2*k + 1) + CATEGORY_EXEMPLARS]])
        except:
            print(j,k)
            print("error!")
            break
        
    submatrix_confounds = temp[1:]
